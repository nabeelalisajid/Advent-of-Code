...
instructions.forEach((intruction) => {
    ...
    const slice = stacksWithData[origin].slice(-total)
    console.log(total,"before",stacksWithData[origin], stacksWithData[desitination])
    stacksWithData[origin].splice(stacksWithData[origin].length-total, total);
    stacksWithData[desitination].push(...slice)
    console.log("after",stacksWithData[origin], stacksWithData[desitination])
